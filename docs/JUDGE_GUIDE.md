...
4. 输出可编辑脚本、分镜、时间线、OTIO-like 轨道 JSON、手法迁移配方、样例-结果对比和质量诊断。
...
4. 先看“全要求验收导航”：它按题目任务 1-13 标出当前证据、页面入口和下一步动作。
5. 查看 RAG 技巧命中、手法迁移配方、真实素材资产、素材缺口、迁移映射、竖屏分镜、时间线草案和“时间线交换 JSON”。
6. 点击“一键出片”里的“手法迁移说明片”，用视频证明源样例节拍如何迁移到新片段。
7. 使用自然语言编辑，例如“开头更抓人，并把证据提前”。
8. 点击 Markdown / JSON 导出，或复制“时间线交换 JSON”给外部剪辑/渲染链路继续处理。
...
| 可展示结果 | 竖屏分镜、时间线草案、时间线交换 JSON、Remotion MP4、关键帧、质量报告 |
...
- 项目的核心结构定义、多样例汇总、素材资产盘点、素材槽位、缺口补全、手法迁移配方、时间线交换协议、质量门禁和前端工作流为自主设计与实现。
