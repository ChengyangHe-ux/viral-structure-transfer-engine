...
未配置模型密钥时，系统会使用本地 fallback 策略，保证评审现场能完整跑通样例拆解、结构迁移、素材缺口、时间线、迁移映射、手法迁移配方、真实素材资产盘点、AIGC 补素材任务、OTIO-like 时间线交换 JSON、质量诊断和导出。
...
生成方案后，结果列会出现“时间线交换 JSON”面板。该面板可复制 `viral-structure-transfer.timeline.v1` 载荷，内容包含：

- `tracks`: 画面剪辑轨、口播字幕轨、包装与转场轨。
- `sourceMap`: 样例时间段、可迁移规则、新片段和素材补全动作。
- `material`: 用户资产、槽位覆盖、缺口数量、候选素材。
- `evaluation`: 当前版本评分和提交状态摘要。

这份 JSON 用于证明项目不只是导出 MP4，也能给外部剪辑脚本、Remotion 管线或后续 NLE 适配器继续编辑。
...
| 结果可验证 | 秒级时间线草案、9:16 分镜预览、Remotion 竖屏成片预览、OTIO-like 时间线交换 JSON、版本切换、Markdown/JSON 导出 | “时间线草案”“竖屏分镜预览”“时间线交换 JSON”、`npm run video:render` 和导出按钮 |
...
8. 展示“时间线草案”和“时间线交换 JSON”，说明脚本已经按秒拆成可生产、可继续剪辑的轨道数据。
...
14. 点击 Markdown 导出或复制时间线 JSON，说明导出稿包含样例-结果对比、素材资产、评分证据矩阵，时间线载荷可进入剪辑协作或二期视频合成链路。
