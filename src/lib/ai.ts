...
function buildVisionAnalysisPrompt(input: {
  ...
  const frameInstruction = input.frameImages.length
    ? `我已经附上 ${input.frameImages.length} 张从样例视频抽取的关键帧。请直接观察这些画面，结合描述判断镜头主体、人物/手部/产品/界面关系、字幕/包装、构图、景别、镜头运动、节奏和卖点推进；不要只复述元数据。`
...
重点拆解：
1. 开头 hook、镜头节奏、字幕布局、画面包装、音乐卡点、卖点推进、结尾转化。
2. 人物/主体要求：是否真人出镜、手部操作、非可识别人物、产品主导或界面录屏主导。
3. 镜头语言：景别、构图、机位、运动方式、快切/停顿/卡点。
4. 可迁移方法与不可复制边界：只抽象出镜方式、节奏和叙事，不复制样例人物身份、品牌、商品、场景、台词和字幕原句。`;
}
...
请额外标注每段人物/主体要求，例如真人口播、手部操作、产品主导、界面录屏、非可识别人物或无人物，并写清哪些具体人物、品牌、商品、场景和台词不能复制。`,
          },
        ],
      },
    ],
...
请只输出画面观察笔记，覆盖：主体/产品、字幕与标题、构图、色彩、包装元素、可能的镜头节奏、适合迁移的规则。
请补充人物/主体观察：是否真人露脸、手部操作、背影/非可识别人物、产品特写或界面录屏；同时标注不能复制的具体身份、品牌、场景和台词。不要输出 JSON。`,
...
3. 方案应可被创作者直接二次编辑。
4. 输出侧重结构迁移，不要复刻样例中的具体人物、台词和画面；每段画面建议必须写清人物/主体要求，例如真人口播、手部操作、产品主导、界面录屏或无人物。
5. 必须输出 3 个版本，每个版本至少 5 个 scriptBeats，第一段 timeRange 必须从 0- 开始。
6. 主版本开头口播必须包含“别”“变化”“错”或“关键”之一，以形成强 Hook。
7. 每个版本必须写出证据/对比/反馈/评价中的至少 2 类；每段 replaceableAssets 必须明确“可替换素材”，并标注图片/视频/文案/链接/AIGC 哪些素材可以承接；每段 riskNotes 必须包含“避免”或“追溯”。
