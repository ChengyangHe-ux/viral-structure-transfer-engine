  ...
  const items: ContestRequirementCoverageItem[] = [
    {
      ...
      evidence:
        ...
        (analysis
          ? `${analysis.sampleTitle} 已拆成 ${analysis.beatMap.length} 个节拍，含 Hook、字幕、包装和音乐线索。`
          : "尚未拆解样例。"),
      judgePanel: "输入素材 / 媒体解析 / 样例结构拆解 / 结构指纹",
      nextAction: analysis ? "演示时先点预设再点“拆解成结构卡片”。" : "选择预设或上传/填写一个样例。",
    },
    {
      taskId: "任务2",
      priority: "P0",
      ...
      evidence:
        ...
        (analysis
          ...
          : "尚未生成结构拆解。"),
      judgePanel: "结构指纹 / 样例结构拆解 / 可迁移节拍",
      nextAction: analysis ? "讲清楚“学结构，不复刻内容”。" : "先完成样例拆解。",
    },
    {
      taskId: "任务3",
      priority: "P0",
      ...
      evidence: material
        ? `Brief 已生成方案；用户素材被拆成 ${materialAssets.length} 个资产和 ${materialSlots.length} 个结构槽位。`
        : plan
          ? "已有新主题 Brief 和方案，等待素材资产诊断。"
          : "尚未输入或生成新内容方案。",
      judgePanel: "多输入闭环 / 迁移 Brief / 用户素材 / 真实素材资产",
      nextAction: material ? "展示多输入闭环和真实素材资产卡片。" : "补充用户素材描述后生成方案。",
    },
    {
      taskId: "任务4",
      priority: "P0",
      ...
      evidence:
        ...
        (plan
          ...
          : "尚未生成迁移方案。"),
      judgePanel: "多版本方案脚本 / 手法迁移配方 / 竖屏分镜预览 / 时间线草案",
      nextAction: plan ? "播放手法迁移说明片或展示迁移映射。" : "点击“生成迁移方案”。",
    },
    {
      taskId: "任务5",
      priority: "P0",
    ...
    },
    {
      ...
      evidence:
        ...
        (techniqueTransfer
          ...
          : "尚未生成手法迁移配方。"),
      judgePanel: "手法迁移配方 / 迁移映射 / 手法迁移说明片",
    ...
    },
    {
      ...
      nextAction: hasStoryboardOrTimeline ? "演示高质量有声或手法迁移说明片。" : "生成迁移方案。",
    },
    {
      taskId: "任务9",
      priority: "P1",
      ...
      evidence:
        ...
        (plan
          ...
          : "尚未生成包装建议。"),
      judgePanel: "手法迁移配方 / 多版本方案脚本 / 竖屏分镜预览",
    ...
    },
    {
      ...
      nextAction: plan ? "切换不同版本展示策略差异。" : "生成迁移方案。",
    },
    {
      taskId: "任务11",
      priority: "P1",
    ...
    },
    {
      ...
      nextAction: plan ? "点击“进入编辑”，现场修改一个 beat。" : "先生成迁移方案。",
    },
    {
      taskId: "任务13",
      priority: "加分",
