...
3. 输入新主题或商品 Brief。
4. 输入用户已有素材或素材描述，并识别结构槽位缺口。
5. 生成 3 个可比较的视频方案脚本版本。
6. 导出 Markdown 和 JSON，方便继续在 Obsidian 或其他工具中编辑。
...
- 接入 ASR 自动转写样例视频口播。
- 已将抽帧结果作为多模态图片输入传入 `AI_MODEL_VISION`，让大模型直接观察样例关键帧；后续可继续增加 ASR 和更密集的镜头采样。
